...
# How many accounts the canary dispatches to check dibycap really is back
# before the full pass commits to it. Three, because one dead cookie must not
# read as an outage and the whole point is to be cheaper than what follows:
# the farmsync walk it guards costs 113 seconds on a farm of 2,148 accounts,
# and a failed dispatch was measured moving the balance by $0.0000.
CANARY_ACCOUNTS = 3
...
def serve(farm, state, work, health, alerter, depletion, progress, titlebar,
    ...
    latest_status = None
    last_status_at = 0.0
    # What the canary dispatches after a pause. Kept from the last pass because
    # the whole point is to answer before walking farmsync again, so it cannot
    # be a fresh fetch. Empty until the first pass has planned one, which is why
    # `canary` treats an empty list as "carry on".
    last_queued = []
    ...
    try:
        while True:
            ...
            # stopped itself on.
            # Read before the call, which is the only place that clears it.
            was_paused = health.open
            ...
            # A pause that has just been lifted has been lifted on the word of
            # `POST /balance`, which does not solve anything. Ask the endpoint
            # that does, before spending two minutes in farmsync on the answer.
            # Skipped in one-shot mode, which has no next refresh to retry in.
            if was_paused and not one_shot:
                Output.info(f"checking dibycap with {CANARY_ACCOUNTS} accounts "
                            f"before fetching the farm (free)")
                probe_worker = Roblox(lock, work, counts, state, health,
                                      dispatched)
                if not canary(probe_worker, last_queued, health):
                    Output.error("dibycap is still failing solves — holding the "
                                 "pool, not fetching the farm")
                    health.trip()
                    # Straight back to the top. `react_to_health` owns the
                    # waiting, and its delay has already doubled, so this is a
                    # backoff rather than a spin.
                    continue
...
def poll_credit(health, depletion, now):
    ...
    try:
        ...
        return None
    # Deliberately no `record_success` here. This read is `POST /balance`;
    # solving is `POST /createTask`, and during a live 503 outage the first
    # answered fine while the second failed for every account. Forgiving the
    # breaker's count on this evidence wiped the run of failures every 60
    # seconds and helped hold the pool in the outage instead of out of it.
...
def canary(worker, accounts, health, count: int = CANARY_ACCOUNTS) -> bool:
    """Is dibycap really solving again? Dispatch a few accounts and find out.

    The recovery probe reads `POST /balance`; solving happens on `POST
    /createTask`. Those failed independently in a live outage — balance
    answered on the first 10-second probe while every solve still came back
    HTTP 503 — so the breaker closed, the producer spent 113 seconds walking
    farmsync, dispatched into the same wall, and paused again. Every cycle.
    This asks the question the probe cannot.

    True means "go on with the pass", and an empty account list returns True:
    the first refresh of a run has nothing to canary with, and refusing to work
    because we could not check is worse than the check being skipped.
    """
    before = health.consecutive
    ran = 0
    for account in accounts[:count]:
        if health.stopped():
            return False
        outcome = worker.dispatch_one(account)
        ran += 1
        # Anything that is not a failure went through dibycap and came back, so
        # the service is answering. That is the whole question.
        if outcome != "fail":
            return True
    if ran == 0:
        return True
    # All of them failed, which on its own proves nothing — three banned
    # accounts fail too, and say nothing about dibycap. Only a failure the
    # breaker counted is service-shaped, so compare against the count it kept
    # rather than re-deciding here.
    return health.consecutive - before < ran


def _probe_countdown(titlebar):
    """What to show while the recovery prober is asleep.

    Two audiences, two costs. The terminal title is free to repaint, so it
    carries the live countdown; scrollback is not, so it gets one line per
    wait — printing every tick would bury the outage under its own countdown.
    A new wait is recognised by the remaining time going *up*, which is the
    only moment the ladder steps.
    """
    previous = [None]

    def on_wait(remaining):
        if previous[0] is None or remaining > previous[0]:
            Output.info(f"waiting for dibycap — next check in {remaining:g}s")
        previous[0] = remaining
        if titlebar is not None:
            titlebar.update(f"FarmsyncSolver | PAUSED — dibycap down | "
                            f"next check in {remaining:g}s")
...
def react_to_health(health, alerter, depletion, status, poll_seconds, wait=True,
    ...
    if health.open:
        ...
        health.recover(solver.status,
                       tick=health_mod.TICK_SECONDS,
                       on_wait=_probe_countdown(titlebar),
