...
# Where an existing install might be sitting, relative to the .exe. The repo
# shape comes first because that is the documented upgrade path: drop the .exe
# into the project folder and run it once. The flat shape is for an operator
# who copied the two files next to the .exe by hand.
CONFIG_CANDIDATES = (Path("input") / "config.json", Path("config.json"))
DB_CANDIDATES = (Path("data") / "state.db", Path("state.db"))


def migrate(source_dir, config_dest, db_dest) -> list:
    """Copy an existing install's two files in from beside the .exe.

    Copies rather than moves, deliberately: a migration that half-finished
    must be recoverable by running the tool again, and an operator who is
    still running the source version must not have it broken underneath them.

    The two files migrate independently. Someone who kept their config but
    deleted state.db gets the config anyway; requiring both would strand them
    on the wizard for no reason.

    A destination that already exists is never overwritten — the live config
    wins over whatever is lying next to the .exe, or every launch from the
    project folder would clobber the operator's edits.

    The source version must not be running while this happens. ``shutil.copy2``
    copies ``state.db`` and nothing else, and ``state.py`` leaves SQLite in its
    default journal mode, so a copy taken mid-write arrives without the journal
    that would complete or undo it. The caller says so out loud; there is
    nothing here that can detect it.

    Returns one human-readable line per file copied.
    """
    source_dir = Path(source_dir)
    copied = []
    for label, candidates, dest in (
            ("config", CONFIG_CANDIDATES, Path(config_dest)),
            ("account state", DB_CANDIDATES, Path(db_dest))):
        if dest.exists():
            continue
        for relative in candidates:
            found = source_dir / relative
            if not found.is_file():
                continue
            dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(found, dest)
            copied.append(f"{label} from {found}")
            break
    return copied
...
def prepare(argv, ask=input) -> bool:
    ...
    everything, migration only runs for a packaged build, and the wizard is
    the last resort.
    """
    ...
    if not config.exists() and paths.portable():
        source = paths.exe_dir() or Path.cwd()
        copied = migrate(source, config, paths.state_db())
        for line in copied:
            Output.info(f"brought forward {line}")
        if copied:
            # state.db is copied without its rollback journal, so a copy taken
            # while the source version is mid-write is a copy of a half-written
            # database. Nothing here can detect that, so it is said instead.
            Output.warn("If the source version is still running, stop it, "
                        "delete the files just copied, and run this again — a "
                        "state.db copied mid-write can be missing its latest "
                        "writes.")
        if config.exists():
            Output.info(f"using {config}")
