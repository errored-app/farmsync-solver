...
# --------------------------------------------------------------------------
# dispatch_one — the canary's seam
# --------------------------------------------------------------------------
#
# `main.canary` reads the returned outcome to decide whether dibycap is really
# solving before the producer spends 113 seconds walking farmsync. A return
# value that stopped arriving would read as "not a failure" and wave every
# outage straight through, which is the exact failure the canary exists to
# stop — and nothing in the output would say so.

@pytest.mark.parametrize("timings,expected", [
    ({"total_ms": 4200, "solve_ms": 1800}, "solved"),
    ({"total_ms": 900, "solve_ms": 0}, "joined"),
])
def test_dispatch_one_hands_back_the_outcome(monkeypatch, no_sleep, account,
                                             timings, expected):
    stub_solve(monkeypatch, timings)
    worker, counts = make_worker([])
    assert worker.dispatch_one(account) == expected
    assert counts[expected] == 1


def test_dispatch_one_reports_a_failure_as_such(monkeypatch, no_sleep, account):
    stub_solve(monkeypatch, SolverError("dibycap unavailable: HTTP 503"))
    health = SolverHealth()
    worker, counts = make_worker([], health=health)

    assert worker.dispatch_one(account) == "fail"
    assert counts["fail"] == 1
    # One dispatch, one count against the breaker — not one per attempt.
    assert health.consecutive == 1


def test_dispatch_one_leaves_the_queue_untouched(monkeypatch, no_sleep, account):
    """It never claimed the account, so it must not release one either.

    A stray release would hand the producer's dedup a key it does not hold.
    """
    stub_solve(monkeypatch, {"total_ms": 900, "solve_ms": 0})
    counts = {"joined": 0, "solved": 0, "fail": 0}
    work = pool_mod.WorkQueue()
    work.submit([account])
    worker = Roblox(lock, work, counts)

    worker.dispatch_one(account)
    # Still waiting for a real worker: the canary borrowed the record, not the slot.
