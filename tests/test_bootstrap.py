"""First-run behaviour: the wizard, and the folder the tool refuses to read.

The packaged build owns exactly one config, at `paths.config_file()`. It used
to also adopt a `config.json` or `input/config.json` found beside the .exe, as
an upgrade path off the source version. That ran in the wild: the .exe sat on a
Desktop that already had an unrelated folder named `input` with a `config.json`
in it, and the tool silently started up on a stranger's credentials instead of
asking. The Desktop is the likeliest folder on the machine to hold a stray
config, so the upgrade path cost more than it bought and was removed.

The two tests below are the shape of that failure, kept so it cannot return.
"""
...
def stub_startup(monkeypatch, exe_dir):
    """Everything `prepare` does before it looks for a config."""
    monkeypatch.setattr(bootstrap.paths, "frozen", lambda: True)
    monkeypatch.setattr(bootstrap.paths, "exe_dir", lambda: exe_dir)
    monkeypatch.setattr(bootstrap.update, "sweep", lambda *_a, **_k: None)
    monkeypatch.setattr(bootstrap, "maybe_update", lambda *_a, **_k: True)


@pytest.mark.parametrize("relative", ["input/config.json", "config.json"])
def test_a_config_beside_the_exe_is_never_adopted(monkeypatch, tmp_path,
                                                  relative):
    """Both shapes the old migration accepted. Neither may be read now.

    The assertion that matters is that the wizard *ran*: a tool that starts on
    a config the operator did not write says nothing is wrong, and the only
    visible symptom is credentials they do not recognise.
    """
    user_dir = tmp_path / "LocalAppData"
    user_dir.mkdir()
    monkeypatch.setenv(paths.OVERRIDE_ENV, str(user_dir))

    desktop = tmp_path / "Desktop"
    stray = desktop / relative
    stray.parent.mkdir(parents=True, exist_ok=True)
    stray.write_text('{"api_key": "someone-elses"}', encoding="utf-8")
    stub_startup(monkeypatch, desktop)

    asked = []

    def ask(prompt):
        asked.append(prompt)
        raise EOFError               # abandon the wizard on its first question

    assert bootstrap.prepare([], ask=ask) is False
    assert asked, "no question was asked — a config was adopted instead"
    assert not (user_dir / "config.json").exists()
    assert stray.exists(), "the stray file was touched"


def test_the_migration_helper_is_gone(tmp_path):
    """Stated as an attribute check, not just an absence of callers.

    Leaving `migrate` importable but unwired is how a later change puts it back
    without anyone deciding to — the tests above would still pass.
    """
    assert not hasattr(bootstrap, "migrate")
    assert not hasattr(bootstrap, "CONFIG_CANDIDATES")
    assert not hasattr(bootstrap, "DB_CANDIDATES")
